...
//--------------------------------------------------------
// Sphere
//http://hu.wikipedia.org/wiki/Ikoza?der
//--------------------------------------------------------
...
struct Wrapper : public Object
{
	...
	Wrapper(int count = 50) : counter(count)
	{
		pedal = false;
...
struct SpaceShip : public Object
{
	...
	SpaceShip(const Vector& pos = Vector(0.0f, 0.0f, 3.0f), const Vector& rot = Vector(0, 0, 0), const Vector& scale = Vector(0.7, 0.7, 0.7), const float ag = 0)
	{
		Position = pos;
		Rotation = rot;
		Scale = scale;
...
struct Camera
{
	...
	Camera() : Position(0.0f, 0.0f, 3.0f), viewDirection(0.0f, 0.0f, -1.0f), up(0.0f, 1.0f, 0.0f)
	{
	...
	//--------------------------------------------------------
	// Hooke
	// source: Alex Hudson-Rex Nelson: ?tban a modern fizik?hoz
	//--------------------------------------------------------
	void Hooke()
	{
		float fi = ((glutGet(GLUT_ELAPSED_TIME) - springstart))*0.05f;


		if (direction)
		{
			if (Position.z <= RopeMin)
				direction = false;
			else
			{
				if (Position.z >= RopeMax)
					Position.z += cosf(ToRadian(fi))*velocity;
				else
					Position.z -= velocity*(glutGet(GLUT_ELAPSED_TIME) - prevTime);
			}
		}
		else
		{
			...
			{
				Position.z += velocity*(glutGet(GLUT_ELAPSED_TIME) - prevTime);
				springstart = glutGet(GLUT_ELAPSED_TIME);
			}
		}
	}
...
void InitObjects()
{
	...
	sun.Position = Vector(0, 0.0f, -33.0f);
...
void onDisplay() {
	globalelapsed = glutGet(GLUT_ELAPSED_TIME)*(360.0f / 9000.0f);
	...
	float Position[] = { 0.0f, -10.0f, -2.0f, 0.0f };
...
void onKeyboard(unsigned char key, int x, int y) {
	if (key == 'w' || key == 'W') spaceship.Wpressed();
	if (key == 'a' || key == 'A') spaceship.Apressed();
	if (key == 's' || key == 'S') spaceship.Spressed();
	if (key == 'd' || key == 'D') spaceship.Dpressed();
...
{
	if (key == 'w' || key == 'W') spaceship.Wreleased();
	if (key == 'a' || key == 'A') spaceship.Areleased();
	if (key == 's' || key == 'S') spaceship.Sreleased();
	if (key == 'd' || key == 'D') spaceship.Dreleased();
...
void onIdle() {
	if ((glutGet(GLUT_ELAPSED_TIME)-prevTime)>(1000/60))
	glutPostRedisplay();
}
