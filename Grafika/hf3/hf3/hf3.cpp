...
struct Sphere: public Object
{
	...
	Sphere(const Vector& pos = Vector(0, 0, 0), const Vector& rot = Vector(0, 0, 0), const Vector& scale = Vector(1, 1, 1),const float ag=0, const int d=5)
	{
		Position = pos;
		Rotation = rot;
		Scale = scale;
...
struct SpaceShip : public Object
{
	...
	SpaceShip(const Vector& pos = Vector(0.5f, 0, 0.3f), const Vector& rot = Vector(1, 0, 0), const Vector& scale = Vector(0.7, 0.7, 0.7), const float ag = 30)
	{
		Position = pos;
		Rotation = rot;
		Scale = scale;
...
struct Camera
{
	...
	{ 
		Delta = Vector(MousePosition.x - x, MousePosition.y - y);
		MousePosition.x = x;
		MousePosition.y = y;
	...
	void RotateCamera()
	{
		if (Delta.x > 10 || Delta.y > 10) 
			return;
		Vector rotate = up%viewDirection;	
		glRotatef(MousePosition.x, up.x, up.y, up.z);
		glRotatef(MousePosition.y, rotate.x, rotate.y, rotate.z);
	}
	void MoveForward()
	{
		Position = Position + viewDirection*MoveSpeed;
	}
	void StrafeLeft()
	{
		Position = Position + (up%viewDirection)*MoveSpeed;
	}
	void MoveBackward()
	{
		Position = Position - viewDirection*MoveSpeed;
...
void InitObjects()
{
	...
	sun.Position = Vector(5.0f, 2.0f, -6.0f);
	sun.Scale = Vector(3.0f, 3.0f, 3.0f);
	aero.Position = Vector(-1.5f, -1.0f, 0.0f);
	planet.Position = Vector(-1.5f, -1.0f, 0.0f);
	aero.Scale = Vector(1.05f, 1.05f, 1.05f);
}
